...
public class SessionService {
    ...
    public SessionDTO createNew(SessionDTO sessionRequest, Integer applicationId) {
        try (Connection dbConnection = DataSource.getConnection()) {
            ...
            // List<ApplicationEvent> applicationEvents = DataFactory.createApplicationEvents(sessionRequest.getHeartBeat(), null);
            // ApplicationEvent heartBeat = DataFactory.createHeartBeatEvent(sessionRequest.getHeartBeat(), null);
            // if (heartBeat != null) {
            ...
            if (sessionRequest.getHeartBeat() != null) {
                heartBeatService.send(new HeartBeatMessage(session.getId(), sessionRequest.getHeartBeat()));
            }
    ...
    public int cleanup() {
        try (Connection dbConnection = DataSource.getConnection()) {
            DataStore dataStore = new DataStore();
            int deletedRows = dataStore.delete(SQLPredicateBuilder.newPredicate().attribute("created_at").lessThan().expression("NOW() - INTERVAL '1 week'"), Session.class, dbConnection);
            dbConnection.commit();
            return deletedRows;
