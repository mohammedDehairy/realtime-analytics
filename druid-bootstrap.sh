...
DRUID_ADMIN_PASSWORD="${DRUID_ADMIN_PASSWORD:-change-me-druid-admin-password}"

echo "Waiting for Druid Router at ${DRUID_ROUTER_URL}..."
until curl -u "admin:${DRUID_ADMIN_PASSWORD}" -fsS "${DRUID_ROUTER_URL}/status/health" >/dev/null 2>&1; do
  sleep 5
done

echo "Configuring retention rules for datasource ${DATASOURCE_NAME}..."
curl -u "admin:${DRUID_ADMIN_PASSWORD}" -fsS -X POST "${DRUID_ROUTER_URL}/druid/coordinator/v1/rules/${DATASOURCE_NAME}" \
  -H "Content-Type: application/json" \
  --data-binary "@${RETENTION_RULES_PATH}"

echo "Configuring Kafka supervisor..."
curl -u "admin:${DRUID_ADMIN_PASSWORD}" -fsS -X POST "${DRUID_ROUTER_URL}/druid/indexer/v1/supervisor" \
  -H "Content-Type: application/json" \
